...
<div class="b">
	<center>
<table bgcolor="white" width="900"><tr><th colspan="5"><?php include 'admin_home3.php';?></th></tr>
	<tr><th>House name</th><th>Maintainance name</th><th>Charge</th><th>date</th><th>Status</th></tr>
	<?php
	  $mt=$conn->query("select * from maintainance where mid='$mid'");
	  if($mt->num_rows >0)
	  {
	  	while($res=mysqli_fetch_assoc($mt))
	  	{
	  		...
	  		echo'<tr><td>Workers list:</td></tr>';

	  		$wlist=$conn->query("select * from wormain where mid='$mid'");
	  		if($wlist->num_rows >0)
	  		{
	  			while($list=mysqli_fetch_assoc($wlist))
	  			{
	  			 $wid=$list['wid'];
	  			 $wn=$conn->query("select * from worker where wid='$wid'");
	  			 if($wn->num_rows >0)
	  			 {
	  			 	while($wnam=mysqli_fetch_assoc($wn))
	  			 	{
	  			 		echo'<tr><td>'.$wname['wname'].'</td>';
	  			 		echo'<td>'.$wname['wcno'].'</td></tr>';
	  			 	}
	  			 }
	  			}
	  		}
